...
import {
  schemaRapportComplet,
  ETAPES_RAPPORT,
  type FormRapportCompletInput,
  type FormRapportComplet,
} from "../../schemas/rapport.schema";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-base-content/60">
          Ajoute des rapports mensuels par lot (CSV) ou saisis-en un
          manuellement via le formulaire guidé.
        </p>
      </div>
      ...
      <div role="tablist" className="tabs tabs-box w-fit">
        <button
          role="tab"
          className={`tab gap-2 ${mode === "csv" ? "tab-active" : ""}`}
          onClick={() => setMode("csv")}
        >
        ...
        <button
          role="tab"
          className={`tab gap-2 ${mode === "manuel" ? "tab-active" : ""}`}
...
function ImportCSV() {
  ...
  const erreur = mutation.error as
    | { response?: { data?: { message?: string; details?: string[] } } }
    | undefined;
    ...
    <div className="max-w-xl space-y-4">
      ...
      >
        ...
        <p className="font-medium">
          Glisse-dépose ton CSV ici, ou clique pour parcourir
        </p>
        <p className="text-xs text-base-content/50">
          Un rapport mensuel par ligne — 20 Mo maximum
        </p>
        <input
          ref={inputRef}
          type="file"
      ...
      {resultat && (
        <div className="alert alert-success">
          ...
          <span>
            {resultat.lignesLues} ligne(s) lue(s) — {resultat.inseres} créée(s),{" "}
            {resultat.misAJour} mise(s) à jour.
          </span>
        </div>
      )}
        ...
        <div className="alert alert-error flex-col items-start">
          ...
          {erreur.response?.data?.details && (
            <ul className="list-disc pl-5 text-sm">
              {erreur.response.data.details.slice(0, 10).map((d, i) => (
                <li key={i}>{d}</li>
              ))}
            </ul>
          )}
...
function FormulaireProgressif() {
  ...
  const {
    ...
    defaultValues: {
      ...
      cas_mas_admis: 0,
      cas_paludisme_grave: 0,
      jours_coupure_electricite: 0,
      poches_sang_disponibles_moy: 0,
      jours_rupture_stock_vitaux: 0,
      deces_moins_24h: 0,
      deces_plus_24h: 0,
    },
  });
    ...
    <div className="max-w-3xl">
      <ul className="steps w-full mb-6">
        {ETAPES_RAPPORT.map((e, i) => (
          <li
            key={e.titre}
            className={`step ${i <= etape ? "step-primary" : ""}`}
          >
            {e.titre}
          </li>
        ))}
      ...
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))}>
          ...
          <fieldset className="fieldset rounded-xl border border-base-300 p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div>
                ...
                <input
                  className={`input w-full ${errors.code_hopital ? "input-error" : ""}`}
                  {...register("code_hopital")}
                />
                {errors.code_hopital && (
                  <p className="text-error text-xs mt-1">
                    {errors.code_hopital.message}
                  </p>
                )}
              </div>
              <div>
                ...
                <input
                  className={`input w-full ${errors.nom_hopital ? "input-error" : ""}`}
                  {...register("nom_hopital")}
                />
                {errors.nom_hopital && (
                  <p className="text-error text-xs mt-1">
                    {errors.nom_hopital.message}
                  </p>
                )}
              </div>
              <div>
          ...
          <fieldset className="fieldset rounded-xl border border-base-300 p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                ...
                <input
                  type="number"
                  className={`input w-full ${errors.lits_disponibles ? "input-error" : ""}`}
                  {...register("lits_disponibles")}
                />
                {errors.lits_disponibles && (
                  <p className="text-error text-xs mt-1">
                    {errors.lits_disponibles.message}
                  </p>
                )}
              </div>
              <div>
                ...
                <input
                  type="number"
                  step="0.1"
                  className={`input w-full ${errors.effectif_medecins_etp ? "input-error" : ""}`}
                  {...register("effectif_medecins_etp")}
                />
                {errors.effectif_medecins_etp && (
                  <p className="text-error text-xs mt-1">
                    {errors.effectif_medecins_etp.message}
                  </p>
                )}
              </div>
              <div>
                ...
                <input
                  type="number"
                  step="0.1"
                  className={`input w-full ${errors.effectif_paramedicaux_etp ? "input-error" : ""}`}
                  {...register("effectif_paramedicaux_etp")}
                />
                {errors.effectif_paramedicaux_etp && (
                  <p className="text-error text-xs mt-1">
                    {errors.effectif_paramedicaux_etp.message}
                  </p>
                )}
              </div>
            </div>
        ...
        {etape === 2 && (
          <fieldset className="fieldset rounded-xl border border-base-300 p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-4">
              <div>
                <label className="label">Admissions totales</label>
                <input
                  type="number"
                  className="input w-full"
              ...
              <div>
                <label className="label">Journées d'hospitalisation</label>
                <input
                  type="number"
                  className="input w-full"
              ...
              <div>
                <label className="label">Consultations externes</label>
                <input
                  type="number"
                  className="input w-full"
              ...
              <div>
                <label className="label">Passages aux urgences</label>
                <input
                  type="number"
                  className="input w-full"
              ...
              <div>
                <label className="label">Actes chirurgicaux</label>
                <input
                  type="number"
                  className="input w-full"
              ...
              <div>
                <label className="label">Accouchements totaux</label>
                <input
                  type="number"
                  className="input w-full"
              ...
              <div>
                <label className="label">Césariennes</label>
                <input
                  type="number"
                  className="input w-full"
        ...
        {etape === 3 && (
          <fieldset className="fieldset rounded-xl border border-base-300 p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-4">
              <div>
                <label className="label">Cas MAS admis</label>
                <input
                  type="number"
                  className="input w-full"
              ...
              <div>
                <label className="label">Paludisme grave</label>
                <input
                  type="number"
                  className="input w-full"
              ...
              <div>
                <label className="label">Jours coupure électricité</label>
                <input
                  type="number"
                  min={0}
                  max={31}
              ...
              <div>
                <label className="label">Poches de sang (moy.)</label>
                <input
                  type="number"
                  className="input w-full"
              ...
              <div>
                <label className="label">Jours rupture stock vitaux</label>
                <input
                  type="number"
                  min={0}
                  max={31}
              ...
              <div>
                <label className="label">Décès &lt; 24h</label>
                <input
                  type="number"
                  className="input w-full"
        ...
        {etape === 4 && (
          <fieldset className="fieldset rounded-xl border border-base-300 p-4">
            <legend className="fieldset-legend">
              Finances & récapitulatif
            </legend>
            <div className="max-w-xs">
              <label className="label">Recettes FANOME (MGA)</label>
              <input
                type="number"
                className={`input w-full ${errors.recettes_fanome_mga ? "input-error" : ""}`}
                {...register("recettes_fanome_mga")}
              />
              {errors.recettes_fanome_mga && (
                <p className="text-error text-xs mt-1">
                  {errors.recettes_fanome_mga.message}
                </p>
              )}
            </div>
            <p className="mt-3 text-sm text-base-content/60">
              Les variables dérivées (TOM, DMS, taux de mortalité, IGPH...) sont
              calculées automatiquement côté serveur à l'enregistrement.
            </p>
          </fieldset>
        )}
          ...
          {derniereEtape ? (
            ...
            >
              {mutation.isPending ? (
                ...
                <>
                  <Check className="size-4" /> Enregistrer le rapport
                </>
              )}
            </button>
          ) : (
