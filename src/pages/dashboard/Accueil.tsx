...
export default function Accueil() {
  ...
  const regionsARisque = [...(regions ?? [])]
    .sort((a, b) => b.pctHautRisque - a.pctHautRisque)
    .slice(0, 5);
  const regionsPerformantes = [...(regions ?? [])]
    .sort((a, b) => b.igphMoyen - a.igphMoyen)
    .slice(0, 5);

  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-linear-to-r from-teal-700 to-teal-900 p-7 text-white">
        <h1 className="text-2xl font-bold">Vue d'ensemble nationale</h1>
        <p className="mt-1 text-teal-100">
          Centraliser, analyser et visualiser la performance des hôpitaux
          malgaches pour mieux piloter le système de santé et optimiser
      ...
      {!chargementRegions && regions && regions.length > 0 && (
        <div className="grid gap-4 md:grid-cols-2">
          <div className="card border border-base-300 bg-base-100">
            <div className="card-body">
              <h2 className="card-title text-base">
                <TriangleAlert className="size-4 text-error" /> Régions à
                ...
                {regionsARisque.map((r) => (
                  ...
                  >
                    ...
                    <span className="badge badge-error badge-outline">
                      {(r.pctHautRisque * 100).toFixed(0)}% haut risque
                    </span>
                  </li>
                ))}
              </ul>
          ...
          <div className="card border border-base-300 bg-base-100">
            <div className="card-body">
              <h2 className="card-title text-base">
                <CircleCheck className="size-4 text-success-content"/> Régions les plus performantes
              </h2>
              <ul className="space-y-2">
                {regionsPerformantes.map((r) => (
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{
                        color: COULEURS_CATEGORIES["Performance élevée"],
                        borderColor: COULEURS_CATEGORIES["Performance élevée"],
                      }}
                    >
      ...
      {!chargementRegions && (!regions || regions.length === 0) && (
        <div className="alert">
          <span>
            Aucune donnée pour l'instant — importe un CSV depuis la page Import
      ...
      <div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          >
            <MapIcon className="size-4" /> Dashboard régional
          </Link>
          <Link
            to="/dashboard/etablissement"
            className="btn btn-outline justify-start"
          >
            <Building2 className="size-4" /> Fiche établissement
          </Link>
          <Link
            to="/dashboard/prediction"
            className="btn btn-outline justify-start"
          >
            <AlertTriangle className="size-4" /> Prédiction de risque
          </Link>
          <Link
            to="/dashboard/methodologie"
            className="btn btn-outline justify-start"
          >
            <BookOpen className="size-4" /> Méthodologie
          </Link>
        </div>
      </div>
    </div>
